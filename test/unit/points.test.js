...
const {
  private: { pointService, uploadService },
  public: { pointService: publicPointService },
} = require('../../src');
...
describe('Points', () => {
  ...
  describe('fetch redacted points for case', () => {
    ...
    it('returns properly formatted points for a case', async () => {
      const results = await pointService.fetchRedactedPoints([
        currentCase.caseId,
      ]);
    ...
    it('pulls new point from database and checks fields', async () => {
      ...
      const results = await pointService.createRedactedPoint(
        currentCase.caseId,
        point,
      );
      ...
      expect(foundPoint.coordinates).to.be.equal(
        '0101000020E6100000CC46E7FC142F3740EFCA2E185C1452C0',
      );
    });
  });
  ...
  describe('create points from upload points', () => {
    ...
    before(async () => {
      ...
      await publicPointService.createMany(
        uploadPoints,
        currentAccessCode,
        uploadId,
      );
    ...
    it('saves uploaded points', async () => {
      const points = await pointService.createPointsFromUpload(
        currentCase.caseId,
        uploadedPoints,
      );
  ...
  describe('update a point', () => {
    ...
    it('returns properly formatted updated point', async () => {
      ...
      const results = await pointService.updateRedactedPoint(
        updatePoint.id,
        point,
      );
  ...
  describe('update multiple points', () => {
    ...
    before(async () => {
      ...
      const uploadedPoints = await uploadService.fetchPoints(currentAccessCode);
      points = await pointService.createPointsFromUpload(
        currentCase.caseId,
        uploadedPoints,
      );
    });
    ...
    it('returns properly formatted updated point', async () => {
      ...
      const results = await pointService.updateRedactedPoints(pointIds, {
